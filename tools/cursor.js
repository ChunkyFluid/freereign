...
export const cursorTool = {
  ...
  renderControls() {
    ...
    return `
      <div class="control-group">
        <label class="control-label">Cursor Type <span class="control-value" id="cursor-val">${selected}</span></label>
      </div>
      <div class="control-group" style="max-height: 380px; overflow-y: auto; scrollbar-width: thin;">
        <div style="display: flex; flex-direction: column; gap: 4px;" id="cursor-list">
          ${cursors.map(c => `
            <button class="cursor-option ${c.name === selected ? 'active' : ''}" data-cursor="${c.name}" style="
              display: flex; align-items: center; justify-content: space-between;
              padding: 8px 12px; border-radius: 8px; font-size: 0.8rem; font-weight: 500;
              background: ${c.name === selected ? 'var(--accent-gradient-subtle)' : 'transparent'};
              color: ${c.name === selected ? 'var(--accent-primary)' : 'var(--text-secondary)'};
  ...
  renderPreview() {
    return `
      <div id="cursor-preview" style="
        ...
        background: var(--accent-gradient); cursor: ${selected};
      ...
      ">
        ...
        <div id="cursor-name" style="font-size: 1.2rem; font-weight: 700; color: white; position: relative; z-index: 1;">${selected}</div>
  ...
  setState(s) { if (s && s.selected) selected = s.selected; },
